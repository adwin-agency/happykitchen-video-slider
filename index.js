...
// Отрисовка стрелок
	...
	// Video play
	const videoBlocks = document.querySelectorAll('.video-reviews__block');

	for (let i = 0; i < videoBlocks.length; i++) {
		const videoBlock = videoBlocks[i];
		...
		videoBlock.addEventListener('click', function() {
			...
			if (isActive) {
				...
				const activeBlock = document.querySelector('.video-reviews__block.active');

				if (activeBlock) {
					activeBlock.classList.remove('active');
					activeBlock.querySelector('video').pause();
				}
		...
		const progress = videoBlock.querySelector('.video-reviews__progress');
